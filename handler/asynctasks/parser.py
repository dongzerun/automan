...
class Location(dict):
  ...
  def pointerline(self):
    def i():
      for i in xrange(0, self._colno + 19):
        yield " "
      yield "^"
  ...
  def __str__(self):
    ...
    return "%s line %s:%s\n%s\n%s" % (self['file'],
                      ...
                      unicode(self._line,'utf8'), self.pointerline())
...
class SQLParser(object):
  ...
  def p_bit_expr_minus1(self, p):
    """bit_expr : bit_expr  NUMBER """
    if p[2] < 0:
        p[0] = ('-',p[1],-p[2])
    else:
        p[0] = ('+',p[1],p[2])

  def p_bit_expr_minus2(self, p):
    """bit_expr : bit_expr  FLOAT_LIT """
    if p[2] < 0:
        p[0] = ('-',p[1],-p[2])
    else:
        p[0] = ('+',p[1],p[2])
  ...
  def p_bit_expr_null_expr(self, p):
    """bit_expr : NULL
                | null """
    p[0] = p[1]
  ...
  def p_double_precision_float1(self, p):
    """double_precision_type : FLOAT limit_opt"""
    p[0] = 'float'
  ...
  def p_default3(self, p):
    """default : DEFAULT CURRENT_TIMESTAMP"""
    p[0] = {'default':'CURRENT_TIMESTAMP'}

  def p_default2(self, p):
    """default : DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP"""
    p[0] = {'default':'CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'}

  def p_default4(self, p):
    """default : DEFAULT value ON UPDATE CURRENT_TIMESTAMP"""
    p[0] = {'default':'IDENT ON UPDATE CURRENT_TIMESTAMP'}
  ...
  def p_column_comment(self, p):
    """column_comment : COMMENT STRING"""
    p[0] = {'COMMENT':p[2]}

  def p_column_comment_empty(self, p):
    """column_comment : """
    p[0] = {}
  ...
  def p_enum_content(self, p):
    """enum_content : '(' opt_values ')'"""
    p[0] = map(lambda x:{'values': x},p[2]) 

  def p_enum_statement(self, p):
    """enum_statement : ENUM enum_content """
    result = dict({'type':p[1]})
    number = []
    for i in p[2]:
        number.append(i['values'])
    result.update({'values':number})
    p[0] = result
  ...
  def p_create_statement_column(self, p):
    """create_statement : col_name column_statement column_comment"""
    t = p[2].pop('type')

    if len(p[2]) > 0:
      p[0] = ('column',p[1],t,p[2],p[3])
    else:
      p[0] = ('column',p[1],t,p[3])
  ...
  def p_table_option_comment(self, p):
    """table_option : COMMENT ignore_eq STRING"""
    p[0] = ('comment',p[3])
  ...
  def p_table_option_ROW_FORMAT(self, p):
    """table_option : ROW_FORMAT ignore_eq IDENT """
    p[0] = ('auto_increment',p[3])
  ...
  def p_create_table(self, p):
    """create_table : CREATE TABLE if_not_exists IDENT '(' create_statements ')' table_options_opt
                    | CREATE TABLE if_not_exists IDENT '.' IDENT '(' create_statements ')' table_options_opt"""
    ...
    if len(p) == 9:
        ...
        args = dict({'location':self._location(p)},**p[3])
        if len(p[10]) > 0:
            args.update({'options':p[10]})
        if len(args) > 0:
            p[0] = [('create_table',p[4] + '.' + p[6],columns,args)]
        else:
            p[0] = [('create_table',p[4] + '.' + p[6],columns)]
  ...
  def p_add_column(self, p):
    """add_column : ADD ignore_column column_name column_statement column_comment position_opt"""
  ...
  def p_change_engine(self, p):
    """change_engine : ENGINE EQ IDENT """
    p[0] = ('change_engine',None,p[3],{'location':self._location(p)})


  def p_change_column(self, p):
    """change_column : CHANGE ignore_column column_name column_name column_statement column_comment position_opt """
  ...
  def p_modify_column(self, p):
    """change_column : MODIFY ignore_column column_name column_statement column_comment position_opt """
    args = dict({'location':self._location(p)},**p[4])
    args.update(p[5])
    args.update(p[6])
    p[0] = ('change_column',None,p[3],args.pop('type'),args)
  ...
  def p_alter_table(self, p):
    """alter_table : ALTER TABLE IDENT alter_specifications 
                   | ALTER TABLE IDENT '.' IDENT alter_specifications"""
    if len(p) == 5:
        p[0] = [tuple([x[0],p[3]] + list(x)[2:]) for x in p[4]]
    elif len(p) == 7:
        p[0] = [tuple([x[0],p[3] + '.' + p[5]] + list(x)[2:]) for x in p[6]]
...
#  def p_expr_or_default3(self, p):
#    """expr_or_default : NULL """
#    p[0] = 'NULL'
...
#cause old p_replace rule can't parse NULL value, so replace it with insert rule
#discourage..... so sad
  def p_replace(self, p):
    """replace :  REPLACE  INTO insert_table  insert_field_spec  """
    args = dict({'location':self._location(p)})
    #p[0] = [('replace',p[2],p[3]['fields'],p[3]['insert_values'],args)]
  ...
  def p_delete_single(self, p):
    """delete : DELETE delete_options FROM IDENT where rows_limit
              | DELETE delete_options FROM IDENT '.' IDENT where rows_limit"""
    if len(p) == 7:
        args = dict({'location':self._location(p)},**p[2])
        args.update(p[5])
        args.update(p[6])
        ...
        args.update(p[7])
        args.update(p[8])
        p[0] = [('delete',p[4] + '.' + p[6], args)]


#  multi delete table forbidden
...
#    p[0] = [('delete',p[3],args)]
  ...
  def p_ident_to_ident(self, p):
    """ident_to_ident : IDENT TO IDENT
                      | IDENT '.' IDENT TO IDENT
                      | IDENT '.' IDENT TO IDENT '.' IDENT
                      | IDENT  TO IDENT '.' IDENT"""
    if len(p) == 4:
        p[0] = ('rename_table',p[1],p[3],{'location':self._location(p)})
    elif len(p) == 6:
        if p[2] == '.':
            p[0] = ('rename_table',p[1] + '.' + p[3],p[5],{'location':self._location(p)})
        elif p[4] == '.':
            p[0] = ('rename_table',p[1],p[3] + '.' + p[5] ,{'location':self._location(p)})
    elif len(p) == 8:
        p[0] = ('rename_table',p[1] + '.' + p[3],p[5] + '.' + p[7], {'location':self._location(p)})
  ...
  def p_table_factor(self, p):
    """table_factor : IDENT opt_table_alias opt_key_definition
                    | IDENT '.' IDENT opt_table_alias opt_key_definition """
    if len(p) == 4:
        args = p[2]
        if len(p[3]) > 0:
            args['keys'] = p[3]
        ...
        if len(p[5]) > 0:
            args['keys'] = p[5]
        if len(args) == 0:
            p[0] = ('table_ref',p[1] + '.' + p[3])
        else:
            p[0] = ('table_ref',p[1] + '.' + p[3],args)
  ...
  def p_use_db(self, p):
    """use_db : USE IDENT"""
    result = ('use', p[2], dict({'location':self._location(p)}))
    p[0] = [result]

  def p_create_database_opt_empty(self,p):
    """create_database_opt : """
    p[0] = {}

  def p_create_database_opt1(self,p):
    """create_database_opt :  COLLATE  IDENT"""
    p[0] = {'collate':p[2]}

  def p_create_database_opt2(self,p):
    """create_database_opt :  CHARACTER SET IDENT"""
    p[0] = {'character':p[3]}

  def p_create_database_opt3(self,p):
    """create_database_opt : DEFAULT COLLATE  IDENT"""
    p[0] = {'collate':p[3]}

  def p_create_database_opt4(self,p):
    """create_database_opt : DEFAULT CHARACTER SET IDENT"""
    p[0] = {'character':p[4]}


  def p_create_database(self, p):
    """create_database : CREATE DATABASE if_not_exists IDENT create_database_opt"""
    result = ('create_database', p[4], dict({'location':self._location(p)}))
    result[2].update(p[3])
    result[2].update(p[5])
    p[0] = [result]
